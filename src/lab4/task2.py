...
class SurveyProcessor:
    def __init__(self, age_bounds):
        ...
        self.groups = self.create_groups(age_bounds)  # Создаем возрастные группы

    def create_groups(self, age_bounds):
        """Метод для создания список возрастных групп на основе переданных границ"""
        groups = []
        prev_bound = 0
...
class AgeGroup:
    ...
    def add_respondent(self, name, age):
        """Добавляем респондента в список респондентов в виде кортежа"""
        self.respondents.append((name, age))

    def print_group(self):
        """Сортируем респондентов внутри группы по убыванию возраста, а при равенстве возраста - по имени в алфавитном порядке и выводим строку с респондентами"""
